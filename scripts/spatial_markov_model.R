# 11/22/23 Adjustments: _init matrices to stabilize burn-in estimation of alphas, normal prior for alphas with variance 0.25

sourceCpp(here("scripts/spatial_markov_model.cpp"))
...
#                                label_unswitch = T, mu_inform = NULL, psi_fix = NULL, alpha_fix = NULL)
...
met_gibbs_potts <- function(y_array, M, W, n_burn_in, n_iter, thin = 1, 
  ...
  alpha_vec <- rep(0, M) # initial alpha1, ..., alphaM
  ...
  samples.alpha <- array(NA, c(n.keep, M-1)) # alpha2, ..., alphaM
  ...
  if (is.null(mu_inform)) {
    bmu <- 1 / apply(y_array, 2, function(mat) var(c(mat), na.rm = T)) # y_array is scaled, so variances are one
  ...
  if (is.null(mu_inform)) {
    amu <- apply(y_array, 2, function(mat) mean(c(mat), na.rm = T)) # y_array is scaled, so means are zero (or virtually zero)
  ...
  accept_psi <- rep(0,2)
  proposal.sd.psi <- 2
  accept_alpha <- matrix(0, nrow = 2, ncol = M-1) # rep(0,2)
  proposal.sd.alphas <- rep(4, M-1)
  ...
  cluster_count_init <- rep(NA, M)
  for (sm in 1:M) {
    ### current cluster count, canonical statistic
    cluster_count_init[sm] <- sum(cluster_mat == sm)
  }
  ...
  for (s in 1:(S + n_burn_in)) {
    ...
    for (sn in 1:N) {
      if (J > 1) {
        cluster_mat[sn, ] <- clusterlabelupdate(W.triplet, W.begfin, W.triplet.sum, nsites = K, y_mat = y_array_da[sn, , ],
                                                mu = mu, J, M, psi, alpha_vec, cluster_mat[sn, ], nu2)
      } else {
    ...
    if (is.null(psi_fix)) {
      ...
      psi_seq_mat <- cbind(matrix(alpha_vec, nrow = length(psi_seq), ncol = M, byrow = T), psi_seq)
      preds_psi <- rep(NA, length(psi_seq))
      offsets_order <- c(order(alpha_vec), M+1)
      ...
      beta <- lm(preds_psi ~ X)$coef
      
      # interpolation
      X1   <- c(psi, (psi^(1:p+1))/(1:p+1))
      logd_current <- -N * sum(X1*beta) # -N * log(d(theta(psi)))
      ...
      logd_proposal <- -N * sum(X2*beta) # -N * log(d(theta(psi)))
    ...
    if (!is.null(alpha_fix)) {
      ...
      for (sm in 2:M) {
        ### current cluster count, canonical statistic
        cluster_count <- sum(cluster_mat == sm)
        if (s <= n_burn_in) {
          cluster_count <- cluster_count_init[sm] # Stabilizing the alpha estimates by freezing cluster counts at those calculated by k-means.
        }
        logprob_alpha_current <- alpha_vec[sm] * cluster_count - alpha_vec[sm]^2/(2*.25)
        alpha_proposal <- rtruncnorm(n=1, a=min_alpha, b=max_alpha, mean=alpha_vec[sm], sd=proposal.sd.alphas[sm-1])
        logprob_alpha_proposal <- alpha_proposal * cluster_count - alpha_proposal^2/(2*.25)
        
        # assembling matrix for prediction
        # combining alpha_seq with other_parms
        alpha_bw <- seq(alpha_vec[sm], alpha_proposal, length = length(psi_seq))
        alpha_bw_mat <- cbind(matrix(alpha_vec, nrow = length(alpha_bw), ncol = M, byrow = T), rep(psi, length(alpha_bw)))
        ...
        hastings <- log(dtruncnorm(x=alpha_vec[sm], a=min_alpha, b=max_alpha, mean=alpha_proposal, sd=proposal.sd.alphas[sm-1])) - log(dtruncnorm(x=alpha_proposal, a=min_alpha, b=max_alpha, mean=alpha_vec[sm], sd=proposal.sd.alphas[sm-1]))
        prob <- exp(-N * logdiff + logprob_alpha_proposal - logprob_alpha_current + hastings)
        # simulated annealing
        prob <- prob^temperature
        ## Accept or reject the proposal
        if(prob > runif(1))
        {
          alpha_vec[sm] <- alpha_proposal
          accept_alpha[1,sm-1] <- accept_alpha[1,sm-1] + 1
        }
        accept_alpha[2,sm-1] <- accept_alpha[2,sm-1] + 1
      }
    }
    ...
    if(ceiling(s/100) == floor(s/100) & s <= n_burn_in)
    {
      ...
      if (is.null(alpha_fix)) {
        for (sm in 2:M) {
          proposal.sd.alphas[sm-1] <- common.accceptrates2(accept_alpha[, sm-1], proposal.sd.alphas[sm-1], 30, 50, sd.max = 5)
          accept_alpha[, sm-1] <- c(0,0)
        }
      }
    }
  ...
  accept_alpha_final <- 100 * accept_alpha[1, ] / accept_alpha[2, ]
  accept_final <- c(accept_psi_final, accept_alpha_final)
  names(accept_final) <- c("psi", paste0("alpha", 2:M))
  ...
  if (length(keep_only) == N) {
    samples <- list(mu = samples.mu, cluster_mat = samples.cluster, 
                    nu2 = mcmc(samples.nu2), psi = mcmc(samples.psi), alphas = mcmc(samples.alpha))
  } else {
    samples <- list(mu = samples.mu, cluster_mat = samples.cluster, 
                    cluster_proportion_array = cluster_proportion_array, nu2 = mcmc(samples.nu2), psi = mcmc(samples.psi), alphas = mcmc(samples.alpha))
  }
